...
function getImportsToResolve(original, includePaths, transformers) {
  ...
  let names = [basename];
  let exts = [extname];
  const extensionPrecedence = [].concat(
    EXT_PRECEDENCE,
...
function* mergeSources(opts, entry, resolve, dependencies, level) {
  ...
  content = content.replace(MATCH_USES, (total, left, file, right) => {
    if (!uses.has(total)) {
      uses.add(total);
    }
...
module.exports = function(content) {
  ...
  return co(function*() {
    ...
    if (cache.isValid()) {
      ...
      merged = Array.from(uses).join("") + merged;
